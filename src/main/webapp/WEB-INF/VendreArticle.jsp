	...
	<div class="container">
		<div class="jumbotron">
			...
			<form action="VendreArticle" method="post" class="d-flex flex-column">
				...
				<c:forEach items="${hasErrors}" var="entry">
					<c:if
						test="${entry.key == 'categorieLibelle' && entry.value==true}">
						<div class="alert alert-danger">La catégorie n'est pas
							valide</div>
					</c:if>
				</c:forEach>
				...
				<label for="debutEnchere">Début de l'enchère</label> <input
					style="width: 200px;" class="form-control" id="debutEnchere"
					name="debutEnchere" type="datetime-local" value="${debutEnchere}">
				<label for="finEnchere">Fin de l'enchère</label> <input
					style="width: 200px;" class="form-control" id="finEnchere"
					name="finEnchere" type="datetime-local" value="${finEnchere}" />

				<c:forEach items="${hasErrors}" var="entry">
					<c:if
						test="${entry.key == 'finEnchereString' && entry.value==true}">
				...
				<fieldset class="d-flex flex-column mt-3">
					...
					<label for="cp">Code postal</label> <input class="form-control"
						id="cp" name="cp" type="text" value="${cp}" />
					<c:forEach items="${hasErrors}" var="entry">
					...
					<label for="ville">Ville</label> <input class="form-control"
						id="ville" name="ville" type="text" value="${ville}" />
					<c:forEach items="${hasErrors}" var="entry">
